...
module.exports = function registerScheduleRoutes(app, { getPool, sql, requireUser, requireAdmin, logAction, actorName, sendInternalError }) {
  app.post("/api/schedules", requireUser, requireAdmin, async (req, res) => {
    ...
    try {
      ...
      const bookingResult = await pool.request().input("BookingId", sql.Int, Number(bookingId)).query("SELECT PreferredDate AS scheduleDate, PreferredTime AS scheduleTime, Status AS status FROM Bookings WHERE Id = @BookingId");
